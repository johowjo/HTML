...
void find_best_hyp(int n, fstream &p10, fstream &p10_u) {
  ...
  for(int i = 0; i < 1; i++) {
    ...
    for(int j = 1; j < theta[i].size(); j++) {
      // if(opt_err > 0.001) cout << j << ' ';
      double pos_dif = 0;
      double neg_dif = 0;
      // for(int k = theta[i][j - 1]; k < theta[i][j]; k++) {
      //   if(train_data[i][k].first == 1) {
      //     pos_dif += u[k] / sum;
      //   } else {
      //     neg_dif += u[k] / sum;
      //   }
      ...
      if(neg_err < opt_err) {
        opt_dim = i;
        opt_err = neg_err;
        opt_t = (double)(theta[i][j - 1] + theta[i][j]) / 2;
        opt_s = -1;
      }
      if(pos_err < opt_err) {
        opt_dim = i;
      ...
      // if (pos_err <= 0.001 || neg_err <= 0.001) cout << "foo";
      // if(opt_err != pos_err && opt_err != neg_err) cout << "error";
      // cout << opt_err
      // cout << neg_err << '\n';
      // if (neg_err >= 0.999) cout << neg_err << ' ';
    }
  }
  ...
  //   if(train_data[opt_dim][i].second < opt_t && opt_s == label[i]) {
  //     // inc.push_back(i);
  //     u[i] *= dia;
  //     e = e + 1;
  //   } else if(train_data[opt_dim][i].second > opt_t && opt_s != label[i]) {
  //     // inc.push_back(i);
  //     u[i] *= dia;
  //     e = e + 1;
  //   } else {
...
//     double score = 0;
//     for(int j = 0; j <= n; j++) {
//       if(s[j] == 1 && train_data[dim[j]][i].second > t[j]) {
//         score += a[j];
//       } else if (s[j] == -1 && train_data[dim[j]][i].second < t[j]) {
//         score += a[j];
//       } else {
//         score -= a[j];
//       }
...
int main() {
  ...
  // for(int i = 0; i < 2000; i++) cout << train_data[0][i].first << ' ';

  double err = 0;
  int count = 0;
  for(int i = 0; i < 2000; i++) {
    if(train_data[90][i] > 476.5 && label[i] == -1) err = err + 1;
    if(train_data[90][i] < 476.5 && label[i] == 1) err = err + 1;
    if(train_data[90][i] == 476) count++;
  }
